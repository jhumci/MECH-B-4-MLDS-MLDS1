...
Der Zentrale Grenzwertsatz besagt, dass die Summe unabhängiger, identisch verteilter Zufallsvariablen (z.B. Stichprobenmittelwerte) eine Normalverteilung annimmt, wenn die Stichprobengröße groß genug ist, unabhängig von der ursprünglichen Verteilung der Daten.
...
* Erweitern Sie das Modell aus Aufgabe 7, indem Sie zusätzlich das Gewicht (`weight`) und Herkunft (aus USA, Europa oder Japan) als Prädiktor für `mpg` aufnehmen.
...
$$
\text{mpg} = \beta_0 + \beta_1 \cdot \text{horsepower} + \beta_2 \cdot \text{weight} + \beta_3 \text{from_US} + \beta_4 \text{from_EU} + \epsilon
$$
Wobei:
...
* $\beta_3$ ist der Koeffizient für die Herkunft des Autos (kategoriale Variable, hier als Dummy-Variablen kodiert: `from_US`).
* $\beta_4$ ist der Koeffizient für die Herkunft des Autos (kategoriale Variable, hier als Dummy-Variablen kodiert: `from_EU`) alle anderen Autos kommen aus Japan.
* $\epsilon$ ist der Fehlerterm.

* **2. Erwartung bezüglich der Koeffizienten für den Intercept und `horsepower`:**
* **Intercept ($\beta_0$):**
    * Der Intercept könnte sich ändern, da wir nun mehr Prädiktoren haben. Der Wert von $\beta_0$ repräsentiert den geschätzten `mpg`, wenn sowohl `horsepower` als auch `weight` Null sind. Da es in der Praxis keine Autos mit 0 PS und 0 Gewicht gibt, ist die Interpretation des Intercepts in diesem Kontext oft weniger sinnvoll. Er dient hauptsächlich dazu, die Regressionsgerade korrekt zu positionieren.
:::
...
* **Interpretierbarkeit in der Originaleinheit:** Das größte Argument für RMSE ist, dass seine Einheit dieselbe ist wie die der abhängigen Variablen, während die Einheit des MSE die quadrierte Einheit ist. Dies macht den RMSE wesentlich **leichter interpretierbar** im Kontext des Problems.
    * Wenn Sie beispielsweise den Preis eines Hauses vorhersagen (in Euro), wäre ein RMSE von 10.000 € intuitiver zu verstehen als ein MSE von 100.000.000 €. Der RMSE gibt direkt an, wie groß die durchschnittliche Abweichung der Vorhersagen in den originalen Einheiten der abhängigen Variablen ist.
...
* **Bias (Verzerrung):**
    ...
    * **Beispiel:** Eine lineare Regression, die versucht, eine nicht-lineare Beziehung zu modellieren oder zu wenige Features verwendet, um die zugrunde liegende Struktur der Daten zu erfassen. 
...
3. Die Richtung der Koeffizienten in der logistischen Regression bleibt ähnlich wie in der linearen Regression: Ein positiver Koeffizient ($\beta_i > 0$) bedeutet, dass eine Erhöhung von $X_i$ die Wahrscheinlichkeit erhöht, dass $Y=1$ (z.B. "Kunde kauft"), während ein negativer Koeffizient ($\beta_i < 0$) die Wahrscheinlichkeit verringert. Allerdings ist der Wert schwer zu interpretieren, da er nicht direkt die Änderung der Wahrscheinlichkeit angibt, sondern die Änderung der Log-Odds.
...
* RMSE $= \sqrt{MSE}$
* MAPE $= \frac{1}{n} \sum |Y_i - \hat{Y}_i|$ (optional, aber hilfreich für die Interpretation).
...
**5. Mean Absolute Percentage Error (MAPE) berechnen (optional):**
$$
MAPE = \frac{1}{n} \sum \left| \frac{Y_i - \hat{Y}_i}{Y_i} \right| = \frac{1}{4} \left( \left| \frac{10 - 11}{10} \right| + \left| \frac{15 - 14}{15} \right| + \left| \frac{20 - 21}{20} \right| + \left| \frac{25 - 23}{25} \right| \right)
$$

Der MAPE hat einen Nachteil, da er bei Werten von $Y_i$ nahe Null sehr große prozentuale Fehlerwerte erzeugen kann. Dies kann zu einer Verzerrung führen, wenn die tatsächlichen Werte sehr klein sind, was die Interpretation des MAPE erschwert. Daher sollte der MAPE mit Vorsicht verwendet werden, insbesondere wenn die Daten Werte nahe Null enthalten. Im Extremfall kann der MAPE sogar unendlich werden, wenn $Y_i = 0$.
:::
